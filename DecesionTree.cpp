...
public:
    ...
    void parse()
    {
        ...
        while (!getline(fin, str).eof())
        {
            ...
            //Adding first row which is represtator of each column.
            if (isAttrName)
            {
                table.attrName = row;
                isAttrName = false;
...
public:
    ...
    string joinByCharacter(vector<string> row, char character)
    {
        string ret = "";
...
int main(int argc, const char *argv[])
{
    ...
    //Printing Headings
    outputPrinter.addLine(outputPrinter.joinByCharacter(test.attrName, ','));

    //Printing data within the headings
    for (int i = 0; i < test.data.size(); i++)
    {
        vector<string> result = test.data[i];
        result.push_back(decisionTree.guess(test.data[i]));
        outputPrinter.addLine(outputPrinter.joinByCharacter(result, ','));
    }
